...
전송 계층은 수신자에게 신뢰할 수 있는 데이터를 전송하기 위해 필요한 계층이다.           
프로토콜마다 다르지만, **오류를 점검하는 기능을 가지고 있어 데이터 전송의 오류 발생시 재전송을 할 수 있다.**      
           
**더불어, `출발 포트/목적지 포트`를 헤더에 포함하고 있기에 데이터를 알맞는 Application 으로 전달해주기도 한다.**                              
