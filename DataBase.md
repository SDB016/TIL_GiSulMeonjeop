...
기존 파일 시스템에서는 같은 파일이 여러 폴더에 존재할 수 있는 중복과           
이 중복으로 인해 발생하는 데이터의 일관성, 무결성 위배될 가능성이 높았기에 이를 해결하고자 데이터베이스 사용           
```무결성 : 현실 세계의 실제값이 일치하는 정확성을 의미한다```    
...
**디스크 I/O :**       
디스크 헤더를 읽어야할 데이터로 이동시킨 다음 데이터를 읽는 것을 의미한다.         
...
### 인덱스(Index)란 무엇인가?
인덱스 :   
* 컬럼값과 레코드의 주소를 KEY와 VALUE로 묶어서 페이지를 만드는 것이다.                 
...
동적인 스케일 아웃을 지원하기도 하며, 
가용성을 위하여 데이터 복제 등의 방법으로 관계형 데이터베이스가 제공하지 못하는 성능과 특징을 제공한다.
...
같은 시간에 조회하는 데이터는 항상 동일한 데이터임을 보증하는 것을 의미한다.       

1. 데이터의 저장 결과를 클라이언트로 응답하기 전에 모든 노드에 데이터를 저장하는 동기식 방법이 있다. (최신화 -> 결과)     
     
2. 메모리나 임시 파일에 기록하고 클라이언트에 먼저 응답한 다음,           
특정 이벤트 또는 프로세스를 사용하여 노드로 데이터를 동기화하는 비동기식 방법이 있다. (결과 -> 최신화)        

</br>

### 2. 가용성(Availability)
    
가용성이란 모든 클라이언트의 읽기와 쓰기 요청에 대하여 항상 응답이 가능해야 함을 보증하는 것    
NoSQL 은 클러스터 내에서 몇 개의 노드가 망가지더라도 정상적인 서비스가 가능하다.    
   
몇몇 NoSQL 은 가용성을 보장하기 위해 데이터 복제(Replication)을 사용한다.     
...
가장 기본적인 형태의 NoSQL 이며          
키 하나로 데이터 하나를 저장하고 조회할 수 있는 단일 키-값 구조를 갖는다.         
복잡한 조회 연산을 지원하지 않으며 고속 읽기와 쓰기에 최적화된 경우가 많다.          
...
하나의 키(ID)에 하나의 구조화된 문서를 저장하고 조회한다.(문서를 저장하니 RDB와 비슷)        
저장된 문서를 컬렉션으로 관리하며 문서 저장과 동시에 문서 ID 에 대한 인덱스를 생성한다.      
...
이러한 컬럼들의 집합이 로우(Row)이며, 로우키(Row key)는 각 로우를 유일하게 식별하는 값이다.        
