...
## 스케줄러
스케줄러 : 어떤 프로세스에게 자원을 할당할지를 결정해주는 운영체제 모듈       
...
실행되는 **코드의 전부를 물리 메모리에 존재시켜야** 했고,   -> 가끔 사용되는 것도 올려야함         
...
초기에 필요한 것들만 적재하는 전략을 `요구 페이징`이라 하며,   
