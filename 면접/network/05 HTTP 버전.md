...
Stream의 유연한 구조 덕분에 응답 frame 들이 요청 순서에 상관없이 만들어진 순서대로 클라이언트에게 전달된다.           
즉, 하나의 TCP 연결을 통해 다수의 클라이언트 요청과 서버 응답이 **비동기 방식으로 이루어지는 멀티플렉싱이 사용된다.**         
물론 필요에 따라 전송 우선순위를 두어 어느정도 전송 순서를 맞추어줄 수 있다.   
(참고로, 각각의 클라이언트를 식별하기 위해 stream에는 식별자를 붙인다.)   
...
그러나 여전히 순서를 보장한다는 특징을 가진 TCP를 사용함으로써,       
1개의 커넥션에서 다수의 요청을 처리한다하여도 첫요청에 대해서 응답이 오지않으면 뒷 요청은 블록킹이 된다.   
