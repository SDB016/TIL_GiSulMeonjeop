...
TCP가 신뢰성있는 전송 프로토콜이라는 것은 아래 기능들 덕분이다.      
각각의 기능들은 TCP Packet에 기능을 수행하기 위한 패킷들을 위치시킨다.    
...
## TCP 재전송 
* selective Repeat와 거의 비슷하다.   
* 송신한 세그먼트의 ACK를 받지 못해도 window 범위 내에서 연속해서 전송(흐름제어)   
* 송신한 세그먼트의 ACK를 timeout가 될 때까지 받지 못하면 재전송한다.     
* 이와 같이 ACK를 받지 못한 세그먼트에 대해서만 재전송한다.    
