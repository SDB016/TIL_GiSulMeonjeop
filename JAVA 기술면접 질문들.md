...
* JAVA와 OS 사이에서 중개자 역할을 수행하여 JAVA가 OS 구애받지 않고 재사용할 수 있게끔 해줍니다.     
* 스택기반의 가상머신입니다.       
...
* 메서드 영역 :        
클래스 파일을 읽어서 분석하여 **초기화되는 대상을 저장하기 위한 메모리 공간**           
클래스, 클래스 변수에 대한 정보 저장         
       
* 힙 :     
인스턴스가 생성되는 공간, 인스턴스 변수들이 생성되는 공간    
        
* 스택 :        
메서드가 호출되면 호출스택에서 준비한 메모리가 할당되고 메서드 작업 종료시 반환되어 비어짐     
...
인터페이스는 추상메서드로 이루어진 것으로           
클래스에서 인터페이스를 구현하여 해당 메서드를 기술해나갑니다.            
자바 8에서는 인터페이스를 구현한 클래스의 기능 확장을 위해 디폴트 메서드와 public static final 변수가 생기기도 했습니다.          
    
abstract 키워드는 메서드 또는 클래스에 붙일 수 있는 키워드로         
해당 요소가 아직 정의 되지 않은 요소임을 나타내줍니다.       
그렇기에 해당 키워드가 붙어있으면 인스턴스를 생성할 수 없으며        
하위 클래스에서 상속받아 해당 요소를 구현하여야지만 인스턴스를 생성할 수 없습니다.           
   
추상 클래스는 클래스에 abstract가 붙은것으로 기존 추상메서드만 있던 인터페이스와 달리    
변수와 정의된 메서드를 생성해도 됩니다.       
다만 추상 메서드를 같이 정의하기에 인스턴스로 생성을 못하고 자식 클래스에서 상속받아 사용하며       
       
인터페이스는 implements로 구현하는 반면에 추상 클래스는 extends로 상속 해야 하고 1개만 상속 가능합니다.   
...
""는 기존 메모리에 할당된 ""을 찾아 참조하게 됩니다.      
만약 기존 ""가 없으면 새롭게 메모리를 할당하여 이를 참조합니다.      
...
JPA는 서로 지향하는 바가 다른 2개의 영역을 중간에서 패러다임 일치를 시켜주기 위한 기술이다.      
...
종합하자면 자바 플랫폼을 위한 오픈소스 애플리케이션 프레임워크로서     
...
Spring은 지원하는 라이브러리, 하위프로젝트들이 늘어나면서 무거워지고 있으며    
초기 설정이 많아 초보자가 처음 프로젝트를 구성하는 데 많은 어려움이 있다.      
그리고 시간이 지나 기술이 발단하고 더 django, ruby on rails 같은 더 경량화된 프레임워크가 등장하여 경쟁이 힘들어졌다.          

스프링 부트는 기존 무거운 스프링 부트를 경량화 하기 위해 만들어졌으며    

# Annotation    
어노테이션이란 본래 주석이란 뜻으로,     
인터페이스를 기반으로 한 메타 데이터의 일종으로          
주석과는 그 역할이 다르지만      
...
- 사용자 정보 파일을 브라우저에 저장하는 쿠키와 달리 세션은 **서버 측에서 관리**합니다.      
...
- Cookie는 쿠키는 클라이언트의 상태 정보를 로컬(클라이언트)에 저장했다가 참조합니다.         
- Cookie는 Client PC에 저장되는 정보기 때문에,    
다른 사용자에 의해서 임의로 변경이 가능하다.(정보 유출 가능, Session보다 보안성이 낮은 이유)        
