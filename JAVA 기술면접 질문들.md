...
이는 그 당시에만 그랬고 매번 프로젝트 전체 설정에 대한 xml을 사용하여 무거워졌고            
지원해주는 라이브러리(하위 프로젝트)가 점차 늘어나서 방대해져서 초기 설정시 많은 어려움이 있었습니다.       
그리고 시간이 지나 기술이 발단하고 더 django, ruby on rails 같은 더 경량화된 프레임워크가 등장하였습니다.       
   
이를 해결하고자 스프링팀에서       

스프링 부트의 아이콘은 이러한 스프링에 봄이 왔다고 하여 벚꽃모양을 사용하는 것으로 알 고 있습니다. 
